...
function CustomTooltip({ active, payload, label }) {
  ...
  return (
    <motion.div
      className="bg-slate-900 text-white rounded-lg shadow-lg p-3 min-w-[180px] border border-slate-800"
      initial={false}
...
function PieTooltip({ active, payload }) {
  ...
  return (
    <motion.div
      className="bg-slate-900 text-white rounded-lg shadow-lg p-3 min-w-[160px] border border-slate-800"
      initial={false}
      animate={{ opacity: 1, y: 0, scale: 1 }}
      transition={{ duration: 0.15 }}
    >
  ...
  return (
    <Card ref={containerRef} className="h-[360px]">
      ...
      <div className="h-[280px]">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <XAxis
              dataKey="month"
              tick={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, fill: '#94a3b8' }}
              tickLine={false}
              axisLine={false}
              axisTick={false}
            />
      ...
      <div className="flex items-center gap-4 mt-4">
        {[
          { label: 'Pemasukan', color: '#16a34a' },
          { label: 'Pengeluaran', color: '#dc2626' },
        ].map((item) => (
          <motion.span
            key={item.label}
            ...
            initial={false}
            animate={{ opacity: 1, x: 0 }}
            transition={{ delay: hasAnimated ? 0.6 : 0, duration: 0.3 }}
          >
            <motion.div
              ...
              initial={false}
              animate={{ scale: [0, 1] }}
              transition={{ delay: hasAnimated ? 0.7 : 0, type: 'spring', stiffness: 200 }}
            />
            {item.label}
          </motion.span>
        ))}
          ...
          {data.slice(0, 6).map((item, i) => {
            ...
            return (
              <motion.div
                ...
                initial={false}
                animate={{ opacity: 1, x: 0 }}
                transition={{ delay: hasAnimated ? 0.5 + i * 0.06 : 0, duration: 0.3 }}
              >
                <motion.div
                  ...
                  initial={false}
                  animate={{ scale: [0, 1] }}
                  transition={{ delay: hasAnimated ? 0.6 + i * 0.06 : 0, type: 'spring', stiffness: 200 }}
                />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-slate-700 truncate">{item.name}</p>
                  <p className="text-xs text-slate-500">{formatIDR(item.value)}</p>
                </div>
                <span className="font-mono font-semibold text-slate-900 tabular-nums text-right whitespace-nowrap">{pct}%</span>
              </motion.div>
            );
          })}
        </div>
